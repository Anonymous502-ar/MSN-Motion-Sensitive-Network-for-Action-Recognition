...
def main():
    ...
    args.store_name = '_'.join(['MSN_', args.dataset, args.modality, full_arch_name, args.consensus_type, 'segment%d' % args.num_segments, 'e{}'.format(args.epochs)])
    ...
    for group in policies:
        logger.info(
            ('[MSN-{}]group: {} has {} params, lr_mult: {}, decay_mult: {}'.
    ...
    train_dataset = TSNDataSet(
        ...
        testtransform=torchvision.transforms.Compose([train_augmentation,
                                                  Stack(roll=(args.arch in ['BNInception', 'InceptionV3'])),
                                                  ToTorchFormatTensor(
...
def train(train_loader, model, criterion, optimizer, epoch, logger=None, scheduler=None):
    ...
    for i, (input, target, testinput) in enumerate(train_loader):
